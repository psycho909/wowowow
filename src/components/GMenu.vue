<script setup>
    ...
    const menuFilter = computed(() => {
        ...
        if (pageTypeSeq.value !== 2) {
            let filteredMenu;
            filteredMenu = props.menu
                .filter(item => item.type.includes(pageTypeSeq.value))
                .map(v => ({
                    ...v,
            ...
            for (const element of remainingMenu) {
                if (element.title.startsWith("GSlide")) {
                    // 尋找是否已經建立過「輪播區塊」
                    let group = groupedMenu.find(item => item.title === "輪播區塊");
                    if (!group) {
                        group = { title: "輪播區塊", elements: [], order: [4, 9] };
                        groupedMenu.push(group);
                    }
                    group.elements.push(element);
                } else if (element.title.startsWith("GImg")) {
                    // 尋找是否已經建立過「圖片區塊」
                    let group = groupedMenu.find(item => item.title === "圖片區塊");
                    if (!group) {
                        group = { title: "圖片區塊", elements: [], order: [5, 10] };
                        groupedMenu.push(group);
                    }
                    group.elements.push(element);
                } else if (element.title.startsWith("GVideo")) {
                    // 尋找是否已經建立過「圖片區塊」
                    let group = groupedMenu.find(item => item.title === "影片區塊");
                    if (!group) {
                        group = { title: "影片區塊", elements: [], order: [6, 11] };
                        groupedMenu.push(group);
                    }
                    group.elements.push(element);
                }  else {
