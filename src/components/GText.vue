...
const onSubmit = async () => {
    textData.validMt = true;
    textData.validMb = true;
    textData.validMmt = true;
    textData.validMmb = true;
    if (textData.mt < 0) {
        textData.validMt = false;
    }
    if (textData.mb < 0) {
        textData.validMb = false;
    }
    if (textData.mobile_mt < 0) {
        textData.validMmt = false;
    }
    if (textData.mobile_mb < 0) {
        textData.validMmb = false;
    }
    if (textData.style == "") {
        ...
        <div class="g-text-container" v-if="!textUpdate">
            <div class="g-text__content"
                ...
                <template v-if="textSetting.type == 'collapse'">
                    <div class="g-text__box"
                         :class="[collapseStatus === true || collapseStatus === 'true' ? '' : 'collapse']"
                         v-html="textSetting.text"
            ...
            <template #edit-content>
                <div class="edit-title__box">
                    <div class="edit-title__text">文字區塊
                        <a :href="`https://tw.hicdn.beanfun.com/beanfun/GamaWWW/allProducts/GamaEvent/Text${pageTypeSeq}.html`"
                ...
                <template v-if="textData.type == 'collapse'">
                    <div class="g-edit__row">
                        <div class="g-edit__col">
                            <g-input class="w-auto" label="文字超過:" type="number" min="1"
                                     v-model="textData.collapse.num" />
                            <span>行後進行收合</span>
                        </div>
                        <div class="g-edit__col">
                ...
                <template v-if="textData.type == 'scrollbar'">
                    <div class="g-edit__row">
                        <div class="g-edit__col">
                            <g-input class="w-auto" label="文字超過:" type="number" min="1"
                                     v-model="textData.scrollbar.num" />
                            <span class="flex[1]">行後出現下拉bar</span>
                        </div>
                    </div>
                </template>
                <div class="g-edit__row">
                ...
                <div class="g-edit__row">
                    ...
                    <div class="g-edit__col w50">
                        <g-input label="Mobile間距上:" type="number" v-model="textData.mobile_mt" @change="handleNumber"
                                 warning="間距請勿設定為負值" :valid="textData.validMmt" />
                    </div>
                    <div class="g-edit__col w50">
                        <g-input label="Mobile間距下:" type="number" v-model="textData.mobile_mb" @change="handleNumber"
                                 warning="間距請勿設定為負值" :valid="textData.validMmb" />
                    </div>
                </div>
