...
export const mainStore = defineStore("main", {
  ...
  actions: {
    ...
    async drgAddSubCpt(uid, data, index) {
      if (!uid || !data || index < 0) return;

      const areaIndex = this.content.findIndex((item) => item.uid === uid);
      if (areaIndex === -1) return;

      const area = this.content[areaIndex];
      const { subContent } = area.content;

      // 檢查和調整插入位置
      const adjustInsertIndex = () => {
        let newIndex = index;
        const targetComponent = subContent[newIndex];
        
        if (targetComponent?.component === "GSlogan") {
          if (!data.cpt) {
            newIndex += 1;
          } else {
            return -1; // Signal to abort insertion
          }
        }

        if (!data.cpt && subContent.some(item => item.component === "GBg")) {
          newIndex += 1;
        }
      ...
      area.content.subContent.splice(adjustedIndex, 0, subComponent);
    },
    addCpt(data) {
      var uid = uuidv4();
      if (data.cpt == "GLang") {
